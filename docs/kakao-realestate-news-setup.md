...
- **인천 검단신도시 청약·거래 소식**: `GEOMDAN_QUERY`(기본 `인천 검단신도시 (청약 OR 분양 OR 실거래가) when:1d`)와 `GEOMDAN_ARTICLE_COUNT`(기본 10, 최대 10개)로 별도 조정 가능합니다. 이 메시지는 **오전(08:00) 발송에만** 포함되고, 오후(17:00)에는 일반 부동산 뉴스만 보냅니다. 관련 뉴스가 없는 날은 검단신도시 메시지 자체를 조용히 건너뜁니다. 다이제스트 페이지의 검단신도시 섹션은 두 번의 실행 모두에서 최신 내용으로 갱신됩니다.
...
- 이 자동화는 매일 아침 실행될 때마다 `docs/index.html`에 "오늘의 부동산 뉴스" 페이지를 갱신해서 저장소에 커밋합니다.
- 그 페이지를 **GitHub Pages**로 공개하면 주소가 하나로 고정됩니다 (예: `https://lifelongdt-beep.github.io/game/`). 페이지 내용은 매일 바뀌지만 링크는 항상 같습니다.
...
3. 잠시 후 `https://{GitHub 사용자명}.github.io/game/` 주소로 접속되는지 확인합니다. (아직 워크플로를 한 번도 안 돌렸다면 4단계를 먼저 진행해 `docs/index.html`을 만들어야 페이지가 뜹니다.)
4. 위 "4. 동작 확인"에서처럼 Actions 탭에서 워크플로를 한 번 수동 실행(`Run workflow`)해 `docs/index.html`이 자동 커밋되는지 확인합니다.
...
방장봇 공지 시각은 `docs/index.html`이 이미 그날 내용으로 갱신된 뒤여야 합니다. 워크플로 기본 실행 시각(07:30 KST)보다 방장봇 공지 시각(예: 08:00~09:00)을 넉넉히 뒤로 잡아주세요. (워크플로 실행 시각을 바꾸는 방법은 위 "5. 커스터마이징" 참고.)
