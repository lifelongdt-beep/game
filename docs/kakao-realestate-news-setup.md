...
- **인천 검단신도시 청약·거래 소식**: `GEOMDAN_QUERY`(기본 `인천 검단신도시 (청약 OR 분양 OR 실거래가) when:1d`)와 `GEOMDAN_ARTICLE_COUNT`(기본 10, 최대 10개)로 별도 조정 가능합니다. 이 메시지는 **오전(08:00) 발송에만** 포함되고, 오후(17:00)에는 일반 부동산 뉴스만 보냅니다. 관련 뉴스가 없는 날은 검단신도시 메시지 자체를 조용히 건너뜁니다. `docs/geomdan.html` 페이지는 일반 뉴스 페이지(`docs/index.html`)와 완전히 분리되어 있고, 두 번의 실행 모두에서 최신 내용으로 갱신됩니다.
